...
sumCoef[1,1] + c(-1, 1) * qt(.975, df = fit$df) * sumCoef[1, 2] # -294.4870 -224.7649
# confidence interval for the slope
sumCoef[2,1] + c(-1, 1) * qt(.975, df = fit$df) * sumCoef[2, 2] # 3556.398 3885.651
(sumCoef[2,1] + c(-1, 1) * qt(.975, df = fit$df) * sumCoef[2, 2])/10 # 355.6398 388.5651 # With 95% confidence, we estimate that a 0.1 carat increase in diamond size results in a 355.6 to 388.6 increase in price in (Singapore) dollars.
...
# Q4 - The estimated expected change in mpg per 1,000 lb increase in weight.
# Q5 - 27.57355
fit <- lm(mpg ~ wt, data=mtcars)
...
# Q7 - It would get multiplied by 100.
fit1 <- lm(mpg~wt, data=mtcars); summary(fit1)
fit2 <- lm(mpg~I(wt/100), data=mtcars); summary(fit2)
# Q8 - The new intercept would be b0 - c*b1
c <- 5
cf1 <- summary(fit)$coefficients; cf1
fit4 <- lm(y ~ I(x+c)) # add a constant c
cf2 <- summary(fit4)$coefficients; cf2
b0 <- cf1[1,1]; b1 <- cf1[2,1]; c(b0,b1)
b0 - c*b1
# Q9 - 0.2471672
fit1 <- lm(mpg ~ wt, data=mtcars); a1 <- anova(fit1)
fit2 <- lm(mpg ~ 1, data=mtcars); a2 <- anova(fit2)
a1[2,2]/a2[1,2]
# Q10 - If an intercept is included, then they will sum to 0.
```

Week 3. Multivariable regression analysis, Residuals & Diagnostics
